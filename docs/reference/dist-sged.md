...
distribution. Note that there are different ways to define a skew GED
...
`dsged` computes the density, `psged` the distribution function, `qsged`
the quantile function, and `rsged` generates random deviates.
